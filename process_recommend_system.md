...
위 벡터는 샘플 plan 데이터를 모두 벡터로 시각화 한 이미지이다. **T-SNE**를 활용하여 `1-4`에서 보여진 데이터를 벡터화 하였다.
...
위 벡터는 `2`번에서 언급한 샘플 plan들의 벡터(빨간색)과 사용자가 최근에 방문한 plan들의 벡터(파란색)을 하나의 좌표에 시각화한 이미지이다. 사용자가 방문한 plan들도 **T-SNE**를 활용하여 벡터화 하였다. 이제 이 plan들과 가장 가까운 plan들이 우선적으로 조회되도록 한다.
